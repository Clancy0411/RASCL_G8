...
CENTER_REFERENCE_SWITCH_HOME="${RASCL_CENTER_REFERENCE_SWITCH_HOME:-true}"
HOMING_CENTER_TIMEOUT_S="${RASCL_HOMING_CENTER_TIMEOUT_S:-30.0}"
HOMING_CENTER_MAX_SCAN_COUNTS="${RASCL_HOMING_CENTER_MAX_SCAN_COUNTS:-100000}"
HOMING_CENTER_RELEASE_CONFIRM_SAMPLES="${RASCL_HOMING_CENTER_RELEASE_CONFIRM_SAMPLES:-3}"
HOMING_CENTER_POLL_INTERVAL_S="${RASCL_HOMING_CENTER_POLL_INTERVAL_S:-0.02}"
HOMING_CENTER_POSITION_TOLERANCE_COUNTS="${RASCL_HOMING_CENTER_POSITION_TOLERANCE_COUNTS:-100}"
...
group_homing_bridge() {
  ...
  [[ "$CENTER_REFERENCE_SWITCH_HOME" == "true" ||
    "$CENTER_REFERENCE_SWITCH_HOME" == "false" ]] ||
    die "RASCL_CENTER_REFERENCE_SWITCH_HOME 只能是 true 或 false"
  is_positive_number "$HOMING_CENTER_TIMEOUT_S" ||
    die "RASCL_HOMING_CENTER_TIMEOUT_S 必须是正数"
  is_integer "$HOMING_CENTER_MAX_SCAN_COUNTS" &&
    (( HOMING_CENTER_MAX_SCAN_COUNTS > 0 )) ||
    die "RASCL_HOMING_CENTER_MAX_SCAN_COUNTS 必须是正整数"
  is_integer "$HOMING_CENTER_RELEASE_CONFIRM_SAMPLES" &&
    (( HOMING_CENTER_RELEASE_CONFIRM_SAMPLES > 0 )) ||
    die "RASCL_HOMING_CENTER_RELEASE_CONFIRM_SAMPLES 必须是正整数"
  is_positive_number "$HOMING_CENTER_POLL_INTERVAL_S" ||
    die "RASCL_HOMING_CENTER_POLL_INTERVAL_S 必须是正数"
  is_integer "$HOMING_CENTER_POSITION_TOLERANCE_COUNTS" &&
    (( HOMING_CENTER_POSITION_TOLERANCE_COUNTS >= 0 )) ||
    die "RASCL_HOMING_CENTER_POSITION_TOLERANCE_COUNTS 必须是非负整数"
  ...
  echo "Drive 0-2 中点 Home=$CENTER_REFERENCE_SWITCH_HOME：检测亮灯边沿后继续同向扫描至熄灭，回到亮灯区中点并置零；最大扫描 $HOMING_CENTER_MAX_SCAN_COUNTS counts，超时 $HOMING_CENTER_TIMEOUT_S s。"
  ...
  ros2 launch rascl_description homing.launch.py \
    ...
    center_reference_switch_home:="$CENTER_REFERENCE_SWITCH_HOME" \
    homing_center_timeout_s:="$HOMING_CENTER_TIMEOUT_S" \
    homing_center_max_scan_counts:="$HOMING_CENTER_MAX_SCAN_COUNTS" \
    homing_center_release_confirm_samples:="$HOMING_CENTER_RELEASE_CONFIRM_SAMPLES" \
    homing_center_poll_interval_s:="$HOMING_CENTER_POLL_INTERVAL_S" \
    homing_center_position_tolerance_counts:="$HOMING_CENTER_POSITION_TOLERANCE_COUNTS" \
