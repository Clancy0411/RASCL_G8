...
2026-07-24 为按同一实体参考点重新校正，当前 `tcp_link` 已还原到最早的
`spur_gear_joint` 中心：
...
`[0.16,0.16] m`。当前将该差值作为固定 XY 平移处理，在上一版基础上把整个机械臂
模型的 X、Y 各增加 `0.040 m`；`base_link -> shoulder_joint` 当前为：

```text
xyz = [0.040,0.020,0.057441] m
```

Z、spur gear 中心 TCP、Homing 和驱动映射均未修改。该单点校正还需用至少两个
不共线的额外 XY 位置验证，以排除坐标旋转或比例误差。
...
```text
q=[0,0,0]
TCP=[0.33756,0.01823,0.043001] m

q=[0,+pi/2,+pi/2]
TCP=[0.24756,0.01823,0.293001] m
```

当前 spur gear 中心修改已通过：
...
首先在实际 Home joint state 检查组 `13` 的 spur gear 中心是否接近当前名义值：
