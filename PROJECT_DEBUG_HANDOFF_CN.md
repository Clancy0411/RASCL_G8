...
```text
close 或 c = 最多相对 -110000 counts，接触物体/端点后提前停止并保持
open  或 o = 最多相对 +110000 counts，到达端点后提前停止并保持
+2000       = 从当前位置正向增加 2000 counts
-500000     = 从当前位置反向减少 500000 counts
```

这些都不是绝对 encoder 目标。`close/open` 是接触感知快捷动作：持续跟踪误差达到默认
`2000 counts / 0.04 s` 时，脚本在 drive following error 前保持实测位置，并记录
`SPUR_CONTACT` 和 `SPUR_RESULT outcome=contact_or_endpoint`。`±110000 counts` 是
最大行程，不保证走满。直接输入的有符号 counts 仍要求精确相对运动，不启用接触提前
终止。默认速度为：
...
使目标坐标对应两爪长度中间的悬空抓取中心。当前固定 TCP 为：
...
```text
q=[0,0,0]
TCP=[0.29318978,-0.01580108,0.07181469] m
