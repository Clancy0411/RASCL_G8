...
随后按用户定义，把 TCP 沿夹爪伸出方向（`lowerarm +X`）累计向外移动 `0.040 m`：
先移到两爪长度中间的悬空抓取中心，再在当前移动坐标基础上继续把 X 加 `0.020 m`。
当前固定 TCP 为：

```text
xyz = [0.15478978,0.02881369,0.03193108] m
```
...
```text
q=[0,0,0]
TCP=[0.31318978,-0.01580108,0.07181469] m

q=[0,+pi/2,+pi/2]
TCP=[0.22318978,-0.01580108,0.32181469] m
```
...
一致，不能证明累计 `40 mm` 的物理方向和长度一定等于实机目标点；实机调整后仍应在 Home
