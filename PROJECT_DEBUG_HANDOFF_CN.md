...
```text
close 或 c = 精确相对 -150000 counts
open  或 o = 精确相对 +150000 counts
+2000       = 从当前位置正向增加 2000 counts
-150000     = 从当前位置反向减少 150000 counts
```

组 `15` 的输入仍然不是绝对目标；执行前后用组 `17` 读取以本次 Method 37 零位为基准
的 `absolute_counts`，可据此实验确定开、合位置。`close`、`open` 和自定义 counts
均是精确相对运动：运行前恢复 `0x60E0/0x60E1=1000`，以 `20000 counts/s` 发送 50 Hz
minimum-jerk CSP 轨迹；不检测接触、不提前停止，也不自动调用 close/hold torque guard。
Drive 3 的 URDF、ros2_control、运动学和脚本预检限位已统一为 `[-2*pi,+2*pi]`；这不会
修改驱动器对象 `0x607B/0x607D`。默认速度为：
...
运动时间由 counts 自动计算；`150000 counts` 约需 7.5 秒。组 `15` 可以与 Cartesian
轨迹在同一 CSP 会话中交替使用，但不能并发执行。
...
23 输入目标 XYZ/时间，规划成功后立即执行实机 minimum-jerk
24 Task 1 阶段 1：移动 1
25 Task 1 阶段 2：移动 2 → 临时方格
26 Task 1 阶段 3：方格 3 → 方格 1
27 Task 1 阶段 4：方格 2 → 方格 3
28 Task 1 完整序列：阶段 1 → 2 → 3 → 4
29 Task 2：输入起点 XY 后完成固定目标点的取放
```
...
Task 1 正式执行时，在同一个正常 CSP 会话的 T3 运行组 `28`；它依次运行
`24 → 25 → 26 → 27`。
四组内的点位均固定：除明确标注的下降点为 `10 s` 外，Cartesian 与夹爪 CSP 轨迹均为
`5 s`，前一动作完成后立即开始下一动作；夹爪动作复用组 `15` 的
`close=-150000`、`open=+150000 counts`。
各组没有额外确认；任一点规划或执行失败即停止当前阶段，不继续发送剩余动作。

Task 2 在 CSP 后由 T3 的组 `29` 执行。它只询问起点 `x/y`，计算
`r=sqrt(x²+y²)` 后按全部 `10 s` 的连续动作完成共同取料段
`(x,y,0.10) → (x,y,0.045) → close → (x,y,0.10)`。当 `0.17 ≤ r ≤ 0.20` 时直接走
`(0.18128633,-0.03369372,0.10) → (0.18128633,-0.03369372,0.045)`；当 `r < 0.17` 时走
`(0.15179144,-0.02821182,0.10) → (0.15179144,-0.02821182,0.045)`；当 `r > 0.20` 时走
`(0.21078122,-0.03917562,0.10) → (0.21078122,-0.03917562,0.045)`。后两条路径随后均到
`(0.18128633,-0.03369372,0.045)`；最后共同执行 `open →
(0.18128633,-0.03369372,0.10)`，不插入等待或确认。
...
组 `23` 会依次读取 `x/y/z/时间`，仅在 IK 和 CSV 检查成功后立即执行；规划失败时
不发送实机运动。组 `14`、`9`、`10` 保持原功能，可用于单独检查：
