...
For every Cartesian leg, group `9` uses live feedback and IK to generate a
named offline trajectory segment in
[`task1_output.csv`](src/rascl_wp3_ss26_group8/trajectories/task1_output.csv).
Group `10` then loads, validates, and executes that exact named segment; it
does not replan during execution. Group `28` clears the file once, performs
this plan/append/load/execute process for all four assessed stages, and leaves
all 24 Cartesian arm segments in the same CSV after completion. The fixed
relative-count gripper actions remain interleaved at runtime and are not
duplicated as Cartesian trajectory rows.
...
- `task1_output.csv`: one combined segmented CSV containing all minimum-jerk
  arm trajectories generated by group `28`. Its header is `segment`,
  `time_from_start`, followed by the four controller joints. Each segment has
  its own timestamps starting at zero. A segment is appended after planning,
  then that same segment is read back, validated, and executed. The file is
  cleared only once at the beginning of group `28`, so later legs do not
  overwrite earlier legs.
...
This one command executes all four Task 1 stages. Each arm leg is appended as
a named segment to the single package CSV and then executed by reading that
same segment. When the command finishes, the CSV contains all 24 Cartesian arm
segments. Any failed plan, CSV validation, or endpoint check stops the
sequence while preserving the segments completed before the failure. Groups
`24`-`27` expose the same stages separately for diagnostics; a separately run
stage resets the CSV once and records every arm leg in that stage.
