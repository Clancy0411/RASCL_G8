...
当前 ROS 硬件层名义映射为 `direction=[+1,-1,+1,+1]`、`home_offset_counts=[0,+802816,-802816,0]`。自动 Home 的 raw 开关位置保持约 `[0,0,0,0] counts`，但 ROS/URDF 应显示约 `[0,+pi/2,+pi/2,0] rad`。原 `3588dc98` 的 URDF `q=0` 与 TCP `[0.29756,-0.00177,0.043001] m` 保持不变。名义值基于两个关节恰好 90°，仍须在原物理 URDF 零姿态读取 `0x6064` 做最终实机精标。
