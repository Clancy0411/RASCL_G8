...
## 2026-07-22 TCP 当前测试点二次修正

程序目标 `XYZ=[0.16,-0.16,0.05] m` 时，用户按实体 `Y/X/Z` 顺序测得 `[0.14,-0.16,0.05] m`。按项目实体轴约定，将对应 `base_link` 数值误差 `[-0.020,0,0] m` 转换到 `lowerarm`，得到当前固定 TCP `[0.11478978,0.02881369,0.03193108] m`。新的名义零姿态 TCP 为 `[0.27318978,-0.01580108,0.07181469] m`，自动 Home TCP 为 `[0.18318978,-0.01580108,0.32181469] m`。该修正按用户要求优先当前测试点，Home 和其他姿态必须重新实测。
