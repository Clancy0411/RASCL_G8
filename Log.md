## 2026-07-27 恢复未补偿 CAD 坐标基准

为重新测量原始实体误差，`non_compensation` 分支移除此前的单姿态固定 `tcp_link`、
抓取中心延伸和全部 `base_link -> shoulder_joint` XY补偿；最新 `main` 保持不变。
TCP 恢复为原始 `spur_gear_joint` origin
`[0.13916,0,0.0179] m`，基座恢复为 `[0,0,0.057441] m`。名义
`q=[0,0,0]` TCP 为 `[0.29756,-0.00177,0.043001] m`，自动 Home TCP 为
`[0.20756,-0.00177,0.293001] m`。D3 Homing、CSP、组15和安全保护均未修改。
带补偿的完整版本仍位于 `main`。
