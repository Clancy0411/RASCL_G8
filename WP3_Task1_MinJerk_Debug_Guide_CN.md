...
5. **T3 组 13：查看当前模型 TCP。**
   - 读取 TF `base_link -> tcp_link`；当前 TCP 已还原到
     ...
     `[0.20756,-0.02177,0.293001] m`。记录输出并测量同一实体参考点。
6. **T3 组 14：输入下一目标。**
...
```text
URDF q=[0,0,0,0] TCP                    = [0.29756,-0.02177,0.043001] m
自动 Home（D0–D2）q~=[0,+pi/2,+pi/2] TCP = [0.20756,-0.02177,0.293001] m
...
不能用模型 TF 自身证明实体位置正确。实体坐标按 `Y/X/Z` 记录，所以实体 `+X`
对应程序 `base_link +Y`。项目已把
