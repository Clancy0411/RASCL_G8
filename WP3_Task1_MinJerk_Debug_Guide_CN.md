...
```text
close 或 c = 最多从当前位置相对 +110000 counts；夹住物体后提前停止并保持
...
越界目标。只有 `close` 是接触感知快捷动作：若命令与反馈的持续误差达到默认
`2000 counts / 0.04 s`，脚本会在驱动器置位 following error 前把目标收回到实测位置，
记录 `SPUR_CONTACT`，然后将这次夹持接触视为成功。它的 `+110000 counts` 是最大闭合
...
保持不动。默认平均速度为
`10000 counts/s`，运动时间根据 counts 自动计算；`close` 最长约 11 秒、`open`
约 20 秒，
