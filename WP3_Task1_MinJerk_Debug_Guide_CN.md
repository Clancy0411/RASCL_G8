...
5. **T3 组 13：查看当前模型 TCP。**
   - 当前原始坐标测试版读取 TF `base_link -> spur_gear`；名义 Home 应接近
     `[0.20756,-0.00177,0.293001] m`。该点是CAD齿轮关节原点，不包含此前的
     单姿态TCP、抓取中心延伸或任何基座XY补偿。
6. **T3 组 14：输入下一目标。**
...
同时自动 Home 仍显示为 `+pi/2`。当前原始坐标测试版以 `spur_gear_joint` 原点为
TCP，IK 与 TF 使用相同CAD定义。
...
重新执行 `T1:4 → T2:6→7 → T3:8→13`，确认原始 CAD TCP 后，再执行
`T3:14→9→10`。不得复用修改前的 CSV 或直接执行旧目标授权。

在 `T1` 运行坐标基准、robot description 参数类型和两个硬件功能测试目标：
...
该值由实时 joint state 和原始URDF正运动学计算，不是对实体夹爪位置的外部测量。
当前版本已移除此前的单姿态固定TCP、抓取中心延伸和全部基座XY补偿，最新主线中的
shoulder origin `[0.040,0.020,0.057441] m` 在本分支恢复为
`[0,0,0.057441] m`，目的是重新测量未经补偿的误差。
外部坐标仍按 `Y/X/Z` 记录，但程序目标输入顺序仍是 `X/Y/Z`。每次必须测量实体上的
同一个标记点，并在多个姿态记录误差后再决定新的标定参数。
