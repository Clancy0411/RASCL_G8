...
Drive 3 没有 Home，因此所有动作都以执行时的当前位置为基准。它的 URDF、ros2_control、
运动学和脚本预检限位统一为 `[-2*pi,+2*pi]`；越界目标仍会被拒绝。只有 `close` 是接触
感知快捷动作：若命令与反馈的持续误差达到默认
`2000 counts / 0.04 s`，脚本会在驱动器置位 following error 前把目标收回到实测位置，
记录 `SPUR_CONTACT`，然后将这次夹持接触视为成功。它的 `+500000 counts` 是最大闭合
