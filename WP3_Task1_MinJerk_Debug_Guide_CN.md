...
```text
close 或 c = 最多从当前位置相对 -110000 counts；接触物体/端点后提前停止并保持
open  或 o = 最多从当前位置相对 +110000 counts；到达端点后提前停止并保持
+2000       = 从当前位置正向增加 2000 counts
-500000     = 从当前位置反向减少 500000 counts
```

Drive 3 没有 Home，因此所有动作都以执行时的当前位置为基准，URDF 软件限位仍会拒绝
越界目标。`close/open` 是接触感知快捷动作：若命令与反馈的持续误差达到默认
`2000 counts / 0.04 s`，脚本会在驱动器置位 following error 前把目标收回到实测位置，
记录 `SPUR_CONTACT`，然后将这次接触/端点视为成功。它们的 `±110000 counts` 是最大
行程，不保证每次都走满。直接输入的有符号 counts 仍是精确相对运动，不使用接触提前
终止；不可达目标仍会失败。
