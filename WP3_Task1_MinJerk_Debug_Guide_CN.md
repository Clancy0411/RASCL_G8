...
5. **T3 组 13：查看当前模型 TCP。**
   - 读取 TF `base_link -> tcp_link`；当前 TCP 已还原到
     `spur_gear_joint` 中心，名义 Home 应接近
     `[0.24756,0.01823,0.293001] m`。记录输出并测量同一实体参考点。
6. **T3 组 14：输入下一目标。**
...
```text
URDF q=[0,0,0,0] TCP                    = [0.33756,0.01823,0.043001] m
自动 Home（D0–D2）q~=[0,+pi/2,+pi/2] TCP = [0.24756,0.01823,0.293001] m
...
同时自动 Home 仍显示为 `+pi/2`。固定 `tcp_link` 与 `spur_gear_joint` 中心重合，
IK 与 TF 使用相同定义。
...
```text
[0.24756, 0.01823, 0.293001] m
```

该值由实时 joint state 和 URDF 正运动学计算，不是外部测量。当前 `tcp_link` 直接使用
`spur_gear_joint` 在 `lowerarm` 中的固定原点 `[0.13916,0,0.0179] m`；此前的
单姿态齿轮表面和抓取中心偏移均不再应用。Home 和其他姿态仍须测量同一个实体参考点，
不能用模型 TF 自身证明实体位置正确。当前单点 XY 校正依据是：模型
`[0.12,0.12] m` 对应实体 `[0.16,0.16] m`。因此机械臂模型相对上一版在
`base_link` 的 X、Y 各平移 `+0.040 m`，当前 shoulder origin 为
`[0.040,0.020,0.057441] m`；Z 与局部 TCP 均未修改。该校正必须再用至少两个
不共线的 XY 点复测，确认误差确实是固定平移而不是比例或旋转误差。
