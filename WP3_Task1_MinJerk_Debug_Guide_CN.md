...
5. **T3 组 13：查看当前模型 TCP。**
   - 读取 TF `base_link -> tcp_link`；当前抓取中心 TCP 的名义 Home 应接近
     `[0.22318978,-0.01580108,0.32181469] m`。它在原齿轮表面校准点基础上沿
     夹爪方向累计向外延伸 `0.040 m`，必须记录 Home 新输出并重新测量实体目标点。
6. **T3 组 14：输入下一目标。**
...
```text
URDF q=[0,0,0,0] TCP                    = [0.31318978,-0.01580108,0.07181469] m
自动 Home（D0–D2）q~=[0,+pi/2,+pi/2] TCP = [0.22318978,-0.01580108,0.32181469] m
...
```text
[0.22318978, -0.01580108, 0.32181469] m
```
...
`0.040 m`；Home 和其他姿态必须重新测量，不能用模型 TF 自身证明实体位置正确。
