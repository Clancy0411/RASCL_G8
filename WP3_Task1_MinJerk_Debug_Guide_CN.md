...
若只是单独测试一个 TCP 目标，T3 使用组 `23`；正式执行整个 Task 1 时使用组 `28`。
组 `24 → 25 → 26 → 27` 仍保留，用于单独运行某一个阶段。
...
11. **T3 组 23：输入目标、规划并立即执行。**
   - 这是日常重复发送坐标的推荐组；依次输入 `x/y/z/时间` 后，复用组 `9` 的 IK、
     CSV 和固定板 XY 补偿检查，再复用组 `10` 的 controller、反馈和终点检查。
   - 规划失败或未生成执行授权时不会运动。规划成功后立即执行同一目标，不再需要手动
     输入 `14 → 9 → 10`。
   - 组 `14`、`9`、`10` 保持原功能，仍可用于单独查看目标、轨迹或排错。
12. **T3 组 24–27：Task 1 四个固定阶段。**
   - 组 `24/25/26/27` 分别执行题目定义的阶段 `1/2/3/4`；坐标已固定在脚本中，
     选择组号后立即开始，不要求重复输入或确认。
   - 每一个 Cartesian 点均先规划后执行；除题目明确标注为 `10 s` 的下降点外，
     所有 Cartesian 与夹爪 CSP 轨迹均为 `5 s`。前一动作完成后立即开始下一动作。
   - `close/open` 直接复用组 `15` 的精确相对 `-150000/+150000 counts` CSP 动作。
     阶段中任一点规划或执行失败时，脚本立刻停止该阶段，不会发送后续动作；排除故障后
     重新运行该阶段组。
...
```text
close 或 c = 从当前位置精确相对 -150000 counts
open  或 o = 从当前位置精确相对 +150000 counts
+2000       = 从当前位置正向增加 2000 counts
-150000     = 从当前位置反向减少 150000 counts
```

Drive 3 的绝对 counts 以本次 Method 37 零位为基准；组 `15` 的数值输入仍是相对当前位置
的增量。`close`、`open` 和自定义 counts 都使用正常 `0x60E0/0x60E1=1000`、
`20000 counts/s` 的 50 Hz minimum-jerk CSP 轨迹；不检测接触、不会提前停止，也不会
自动调用 close/hold torque guard。前三轴保持当前 `/joint_states`。精确目标不可达时
动作失败。

组 `17` 可在动作前后读取 `absolute_counts`，确认本次相对增量。组 `15` 完成后会清除
旧轨迹授权；下一次 Cartesian 运动使用组 `23`，或继续使用原来的 `14 → 9 → 10`。
同一 CSP 会话可交替运行：`15 → 23 → 15`，但不能与正在发布的 Cartesian 轨迹并发。
...
组 `23` 依次输入目标、规划并在成功后立即执行。组 `14`、`9`、`10` 仍可分别用
`bash ./rascl_debug.sh <组号>` 运行，用于检查目标或轨迹。规划授权只对仍在运行的当前组
`7` 会话和完全相同的目标有效；组 `7` 退出、改目标或完成一次运动后自动失效。

IK/规划失败但 T1/T2 没有 PDO、WKC、following error，且两个 controller 仍为
`active` 时，不需要重启 CSP；换一个安全目标，重新执行组 `23`（或 `14 → 9`）。
禁止对规划失败的目标执行组 `10`。
...
6. 从 `T1:4 → T2:6→7 → T3:8→13→23` 完整重来。
...
| 23 | T3 | 输入目标、规划成功后立即执行 |
| 24 / 25 / 26 / 27 | T3 | Task 1 的阶段 1 / 2 / 3 / 4 一键动作 |
| 28 | T3 | Task 1 全部四阶段连续执行 |
| 29 | T3 | Task 2：输入起点 XY 后完成固定目标点的取放 |

日常实机顺序以本指南最前面的
`T1:4 → T2:6→（标定时 19/20/21→22）→7 → T3:8→13→28`
为准。下面保留原始命令用于排错。

### Task 2 固定取放

进入 CSP 后，在 T3 执行组 `29`，仅输入起点 `x/y`（米）。脚本自动计算
`r=sqrt(x²+y²)`；为对比抖动，所有动作均为 `10 s`：
...
0.17 ≤ r ≤ 0.20：
→ (0.18128633,-0.03369372,0.10) → (0.18128633,-0.03369372,0.045)

r < 0.17：
→ (0.15179144,-0.02821182,0.10) → (0.15179144,-0.02821182,0.045)
→ (0.18128633,-0.03369372,0.045)

r > 0.20：
→ (0.21078122,-0.03917562,0.10) → (0.21078122,-0.03917562,0.045)
→ (0.18128633,-0.03369372,0.045)

共同结束：open → (0.18128633,-0.03369372,0.10)
```
...
`T3:23`（或 `T3:14→9→10`）。不得复用修改前的 CSV 或直接执行旧目标授权。
...
脚本：日常使用 `T3` 组 `23` 输入目标、规划并执行。若需要单独检查轨迹，仍可依次
选择组 `14`、`9`、`10`。每个新目标都必须重新规划。
