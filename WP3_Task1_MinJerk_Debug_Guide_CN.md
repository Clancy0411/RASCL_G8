...
1. **T1 组 4：启动唯一 EtherCAT/Homing bridge。**
   - 当前工作站使用网卡 `enx94bdbe956314`。脚本组 `4` 会自动使用该默认值；只有切换到另一台工作站时，才通过 `RASCL_INTERFACE=<网卡名>` 覆盖。
...
```bash
ip link show enx94bdbe956314
sudo ip link set enx94bdbe956314 up
ip link show enx94bdbe956314
```
