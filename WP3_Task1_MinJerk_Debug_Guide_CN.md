...
5. **T3 组 13：查看当前模型 TCP。**
   - 读取 TF `base_link -> tcp_link`；当前 TCP 是沿 lowerarm +X 测得的
     ...
     `[0.27840,0.01823,0.293001] m`。记录输出并测量同一实体参考点。
6. **T3 组 14：输入下一目标。**
...
```text
URDF q=[0,0,0,0] TCP                    = [0.36840,0.01823,0.043001] m
自动 Home（D0–D2）q~=[0,+pi/2,+pi/2] TCP = [0.27840,0.01823,0.293001] m
...
同一个理想 TCP 实体参考点，不能用模型 TF 自身证明实体位置正确。当前单点 XY
校正依据是：模型
`[0.12,0.12] m` 对应实体 `[0.16,0.16] m`。因此机械臂模型相对上一版在
`base_link` 的 X、Y 各平移 `+0.040 m`，当前 shoulder origin 为
`[0.040,0.020,0.057441] m`；Z 与全局基座平移均未因本次 TCP 长度测量而修改。
该校正必须再用至少两个不共线的 XY 点复测，确认误差确实是固定平移而不是比例或
旋转误差。
